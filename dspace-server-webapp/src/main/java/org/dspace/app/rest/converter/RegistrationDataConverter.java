...
public class RegistrationDataConverter implements DSpaceConverter<RegistrationData, RegistrationRest> {
    ...
    public RegistrationRest convert(RegistrationData registrationData, Projection projection) {
        ...
        registrationRest.setGroupNames(getGroupNames(registrationData));
        registrationRest.setGroups(
            registrationData.getGroups().stream().map(Group::getID).collect(Collectors.toList())
        );
    ...
    private List<String> getGroupNames(RegistrationData registrationData) {
        return registrationData.getGroups().stream()
                               .map(Group::getName)
